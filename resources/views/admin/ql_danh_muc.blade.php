                    ...
                    <table class="table table-striped table-bordered table-hover" border="0">
                        <tbody>
                            ...
                            <form action="{{url('admin/danh-muc/search')}}" method="get">
                                <td> <a href="{{url('admin/danh-muc')}}" class="btn btn-warning" title="Trở lại">
                                ...
                                <td>  
                                <form action="{{url('admin/danh-muc/select-all')}}" method="Post">
                                ...
                                <td><label
                                    class="btn btn-danger" disabled >Hiện có: {{$data->total()}} danh mục </label>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            @foreach ($data as $item)
                                ...
                                <td scope="row">{{$loop->index+1}}</td>
                                    ...
                                    <td class="center">
                                     <form id="form{{$item->id}}" action="{{url('admin/danh-muc/delete/'.$item->id)}}" method="post">
        ...
        <div class="col-lg-12">
            <div class="panel panel-green">
                ...
                <div class="panel-body">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <form action="{{url('admin/danh-muc/insert')}}"method="post">
            ...
            <div class="panel panel-green ">
                ...
                <div class="panel-body">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <form
                                 action="{{url('admin/danh-muc/edit')}}"
                                method="post">
    ...
    $(document).ready(function(){
        ...
        $(document).on('click','a.btn-sua',function(){
            ...
            $.ajax({
                ...
                {
                            $("#sua #id").val(data.id);
                        $("#sua #ten_danh_muc").val(data.ten_danh_muc);
        ...
        var array_value_checkbox = new Array(); // Mảng các id của danh mục
        ...
        $(document).on('click','input[type=checkbox]',function(){
            ...
            {
                array_value_checkbox.push(id); // Thêm id của danh mục vào arrray
                
            }
            else
            {
                var index_checkbox_unchecked = array_value_checkbox.indexOf(id); // Tìm index của id danh mục trong array
                array_value_checkbox.splice(index_checkbox_unchecked,1); // Xóa id của danh mục trong arrray theo index
            } 
    ...
    CheckAll = (array_checkbox,array_value_checkbox,array_button) => {
        if(array_value_checkbox.length > 0) // Nếu mảng khác rỗng, tức là đã chọn tất cả 
        {
            for(let i = 0; i < array_checkbox.length; i++) // Duyệt mảng các control checkbox
            {
                ...
                var index_checkbox_unchecked = array_value_checkbox.indexOf(array_checkbox[i].getAttribute("id")); // Tìm index của id danh mục trong array 
                array_value_checkbox.splice(index_checkbox_unchecked,1); // Xóa id của danh mục trong arrray theo index
                $("#btn-all").html('Chọn tất cả'); // Đổi tên hiển thị của button 
            }
        ...
        else // Nếu mảng rỗng , tức là chưa chọn gì hết
        {
            for(let i = 0; i < array_checkbox.length; i++)// Duyệt mảng các control checkbox
            {
                array_value_checkbox.push(array_checkbox[i].getAttribute("id")); // Thêm id của danh mục vào arrray
